...
static void pnx_threadx_stack_error(TX_THREAD *thread_ptr)
...
VOID tx_application_define(VOID *first_unused_memory)
{
  ...
  else
  {
    ...
    if (tx_thread_stack_error_notify(pnx_threadx_stack_error) != TX_SUCCESS)
    {
      Error_Handler();
    }
  ...
  /* USER CODE BEGIN PNX_App_Start */
  app_start();
  /* USER CODE END PNX_App_Start */
}

/* USER CODE BEGIN  0 */
TX_BYTE_POOL *pnx_threadx_app_pool(void)
{
...
static void pnx_threadx_stack_error(TX_THREAD *thread_ptr)
{
  __asm volatile("b bsp_diagnostics_capture_threadx_stack");
}
/* USER CODE END  0 */
