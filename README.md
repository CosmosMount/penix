...
`embedded_framework` 是面向 RoboMaster 类机器人控制板的 STM32H723 固件框架。工程以 STM32CubeMX 生成的 HAL/ThreadX/USBX 工程为硬件基础，在其上提供配置驱动的 BSP、设备抽象、可复用业务模块、通用库和板端 demo。
...
| `bsp/` | CAN、USART、SPI、PWM、DWT、EXTI、Flash、USB 等板级外设封装。 |
...
```powershell
cmake -S embedded_framework -B .agents\build\embedded_framework_arm -G Ninja -DCMAKE_TOOLCHAIN_FILE=board/cmake/gcc-arm-none-eabi.cmake
